...
def main():
    ...
    # train_data 0 là chuỗi session, 1 là label của session đó (món đồ được chọn tiếp theo)
    # Ví dụ: train_data[0][0] là chuỗi session đầu tiên,

    if opt.validation:
        # Mặc định điều kiện này sẽ không chạy
    ...
    # Chuyển đổi dữ liệu thành định dạng phù hợp để đưa vào mô hình,
    train_data = Data(train_data, shuffle=True)

    # Tương tự trên chỉ khác là làm việc trên dữ liệu test
    test_data = Data(test_data, shuffle=False)
    # del all_train_seq, g

    # Cấu hình số node, tuy nhiên đây là cách không hiệu quả và linh hoạt, có thể sửa về sau
